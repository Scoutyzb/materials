...
(function($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function parseOptions(opts) {
            ...
            for (i = 0; i < Math.max(1, options.xaxes.length); ++i) options.xaxes[i] = $.extend(true, {},
            options.xaxis, options.xaxes[i]);
            for (i = 0; i < Math.max(1, options.yaxes.length); ++i) options.yaxes[i] = $.extend(true, {},
            options.yaxis, options.yaxes[i]);
                ...
                for (var i = 0; i < axis.ticks.length; ++i) {
                    ...
                    if (axis.direction == "x") {
                        ...
                        if (axis.position == "left") {
                            pos.right = canvasWidth - (box.left + box.width - box.padding) align = "right";
                        } else {
        ...
        function drawBarHighlight(series, point) {
            ...
            var barLeft = series.bars.align == "left" ? 0 : -series.bars.barWidth / 2;
            drawBar(point[0], point[1], point[2] || 0, barLeft, barLeft + series.bars.barWidth, 0,
            function() {
                return fillStyle;
            },
            ...
            if (typeof spec == "string") return spec;
                ...
                l = spec.colors.length; i < l; ++i) {
                    ...
                    if (typeof c != "string") {
                        ...
                        if (c.brightness != null) co = co.scale('rgb', c.brightness) if (c.opacity != null) co.a *= c.opacity;
                        c = co.toString();
                    }
    ...
    $.plot.formatDate = function(d, fmt, monthNames) {
        ...
        var isAM = hours < 12;
        if (monthNames == null) monthNames = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"];
